...
def oil_dry_anls(customer_id):
    ...
    if strip_score:
        if 7 <= strip_score <= 8:
            strip_analysis = STRIP_STATEMENT[0][1]
        elif 5 <= strip_score <= 6:
            strip_analysis = STRIP_STATEMENT[1][1]
        elif 3 <= strip_score <= 4:
            strip_analysis = STRIP_STATEMENT[2][1]
        elif 1 <= strip_score <= 2:
            strip_analysis = STRIP_STATEMENT[3][1]
    
    quiz_score = skin.filter(~Q(choice__question__code__in=[100,101])).aggregate(Sum('choice__marks'))['choice__marks__sum'] or 0.00
    quiz_analysis = ''
    if quiz_score:
        if 16 <= quiz_score <= 20:
            quiz_analysis = QUIZ_STATEMENT[0][1]
        elif 12 <= quiz_score <= 15:
            quiz_analysis = QUIZ_STATEMENT[1][1]
        elif 8 <= quiz_score <= 11:
            quiz_analysis = QUIZ_STATEMENT[2][1]
        elif 5 <= quiz_score <= 7:
            quiz_analysis = QUIZ_STATEMENT[3][1]
